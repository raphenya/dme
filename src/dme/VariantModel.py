        ...
        with open(self.xml_file, 'r') as result_handle:
            ...
            for blast_record in blast_records:
                                ...
                                if hsp.sbjct_start < pos and (hsp.sbjct_start + real_sbjct_length) > pos:
                                    ...
                                    if predicted_genes_dict_protein:
                                        if orf_info.strip() in predicted_genes_dict_protein.keys():
                                            ...
                                            orf_protein_sequence = predicted_genes_dict_protein[orf_info.decode(
                                            )[:orf_info.decode().index(' # ')]].strip("*")
                                            ...
                                            if float(hsp.bits) >= float(true_pass_evalue):
                                                ...
                                                if self.input_type == 'contig':
                                                    ...
                                                    if orf_info.decode().split(' # ')[0] in predicted_genes_dict:
                                                        sinsidedict["orf_dna_sequence"] = predicted_genes_dict[orf_info.decode().split(' # ')[
                                                            0]]
                                                ...
                                                slinsidedict["orf_strand"] = self.extract_nth_bar(
                                                    orf_info.decode(), 0)
                                                slinsidedict["orf_start"] = self.extract_nth_bar(
                                                    orf_info.decode(), 1)
                                                ...
                                                if self.input_type == 'contig':
                                                    ...
                                                    if orf_info.decode().split(' # ')[0] in predicted_genes_dict:
                                                        slinsidedict["orf_dna_sequence"] = predicted_genes_dict[orf_info.decode().split(' # ')[
                                                            0]]
                                                    ...
                                                    slinsidedict["query_end"] = hsp.query_start + \
                                                        real_query_length
                ...
                blastResults = self.results(
                    blastResults, blast_record.query, perfect, strict, loose)
