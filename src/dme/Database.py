...
class Database(object):
	...
	def make_blast_database(self,dbtype="protein"):
		...
		if os.path.isfile(os.path.join(self.db,"{}db.fsa".format(dbtype))) == True and os.path.exists(os.path.join(self.db,"{}db.fsa".format(dbtype))) == True  \
		   ...
		   pass
			...
			if dbtype == "dna":
				type = "nucl"
			os.system('makeblastdb -in {} -dbtype {type} -out {} {stdout}'.format(os.path.join(self.db,"{}db.fsa".format(dbtype)),os.path.join(self.db,"{}.db".format(dbtype)),type=type,stdout=self.stdout))
	...
	def make_diamond_database(self,dbtype="protein"):
		...
		if os.path.isfile(os.path.join(self.db,"{}db.fsa".format(dbtype))) == True and os.path.exists(os.path.join(self.db,"{}db.fsa".format(dbtype))) == True \
			...
			os.system('diamond makedb --quiet --in {} --db {} {stdout}'.format(os.path.join(self.db,"{}db.fsa".format(dbtype)),os.path.join(self.db,"{}.db".format(dbtype)),stdout=self.stdout))
					...
					if i.isdigit():
						...
						if j[i]['model_type_id'] == 54:
							try:
								...
								logger.info("Please let the HMDM Admins know! Email: raphenar@mcmaster.ca")
							else:
								try:
		            	...
		            	# model_type: protein variant model
							...
							try:
								...
								try:
									...
									logger.warning("No snp for model (%s, %s). DME will omit this model and keep running." \
										% (j[i]['model_id'], j[i]['model_name']))
									logger.info("Please let the HMDM Admins know! Email: raphenar@mcmaster.ca")
	...
	def write_fasta_from_json_rna(self):
		snpList_16s = []
		snpList_23s = []
		"""Creates a fasta file for 16S and 23S data from hmdm.json file."""
