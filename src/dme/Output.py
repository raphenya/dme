...
class Output(object):
    ...
    def __init__(self, input_file):
        ...
        self.output_file = os.path.join(
            working_directory, "{}.tab.txt".format(f_name))
    ...
    def print_csv(self):
        if os.path.isfile(self.input_file) == False:
            logger.error(
                "missing input JSON file. ({})".format(self.input_file))
            exit()
                    ...
                    for it in data[item]:
                        ...
                        if self.checkKeyExisted("ARO_category", data[item][it]):
                            for aroctkey in data[item][it]["ARO_category"]:
                                ...
                                cgList.append(str(data[item][it]["ARO_category"][aroctkey]["category_aro_name"].encode(
                                    'ascii', 'replace'), 'utf-8'))

                        if data[item][it]["model_type_id"] == 40293:
                            temp = data[item][it]["SNP"]["original"] + str(
                                data[item][it]["SNP"]["position"]) + data[item][it]["SNP"]["change"]
                            snpList.append(temp)
                        elif data[item][it]["model_type_id"] == 40292:
                            snpList.append("n/a")

                        AROlist.append(
                            str(data[item][it]["ARO_accession"].encode('ascii', 'replace'), 'utf-8'))
                        AROnameList.append(
                            str(data[item][it]["ARO_name"].encode('ascii', 'replace'), 'utf-8'))
                        ...
                        identityList.append(
                            float(data[item][it]["perc_identity"]))
                        bestAROcategory = []
                            ...
                            if maxscore < float(data[item][it]["bit-score"]) and maxpercent < float(data[item][it]["perc_identity"]):
                                ...
                                if self.checkKeyExisted("ARO_category", data[item][it]):
                                    for key in data[item][it]["ARO_category"]:
                                        bestAROcategory.append(str(data[item][it]["ARO_category"][key]["category_aro_name"].encode(
                                            'ascii', 'replace'), 'utf-8'))
                                    bestAROcategorydict[str(
                                        minARO) + "|" + str(minevalue)] = bestAROcategory
                    ...
                    if typeList:
                        if self.findnthbar2(item, 1) == "":
                            writer.writerow([item,
                                             ...
                                             maxpercent,
                                             ', '.join(
                                                 map(lambda x: "ARO:" + x, AROlist)),
                                             '; '.join(list(arocatset)),
                                             ', '.join(list(tl)),
                                             snpList,
                                             '; '.join(bestAROcategorydict[str(
                                                 minARO) + "|" + str(minevalue)]),
                                             '; '.join(AROsortedList),
                                             pass_bitscore,
                                             maxscore,
