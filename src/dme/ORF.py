from dme.settings import os, SeqIO, logger
import tempfile
import time
import fileinput
import math
    ...
    def prodigal_run(self, fasta, *o):
        ...
        for entry in fasta:
            ...
            tmp = os.path.join(self.working_directory,
                               "{}.temp.directory".format(o_f_name))
    ...
    def execute_threads(self, split_sequences):
        ...
        for ind in range(len(split_sequences)):
            process = multiprocessing.Process(
                target=self.prodigal_run, args=(split_sequences[ind], output,))
            process.start()
        ...
        for i in results:
            for j in i:
                ...
                self.write_output_file(output_dna_orf, [
                                       "{wd}/{tmp_name}.temp.contigToORF.fsa".format(wd=self.working_directory, tmp_name=o_f_name)])
                self.write_output_file(output_prot_orf, [
                                       "{wd}/{tmp_name}.temp.contig.fsa".format(wd=self.working_directory, tmp_name=o_f_name)])
                self.write_output_file(output_draft, [
                                       "{wd}/{tmp_name}.temp.draft".format(wd=self.working_directory, tmp_name=o_f_name)])
                self.write_output_file(output_potential_genes, [
                                       "{wd}/{tmp_name}.temp.potentialGenes".format(wd=self.working_directory, tmp_name=o_f_name)])
