...
from dme.settings import SeqIO, logger
...
class Analyser(object):
    ...
    def __repr__(self):
        ...
        return f"Analyser({self.__dict__})"
    ...
    # Using the Biopython fasta parse we can read our fasta input
        for record in SeqIO.parse(self.input_file, "fasta"):
            ...
            if checksum in checksums:
                model_type_id = record.description.split(
                    "|")[1].split(":")[-1].strip()
                if model_type_id in ["40292"]:
                    logger.warning(
                        "Duplicates {%s} -> {%s} ~ {%s} - [{%s}]", record.id,
                        record.description.split(
                            "|")[-1], d[checksum].split("|")[-1],
                        model_type_id)
                continue
