...
import dme.Parser
import dme.load
import dme.auto_load
import dme.clean
import dme.build_kmer_sets
import dme.hmdm_annotation
import dme.wildcard_annotation
import dme.baits_annotation
import dme.remove_duplicates
from dme.kmer_query import CARDkmers
from dme.BWT import BWT
from dme.Heatmap import Heatmap
from dme.Baits import Baits
import importlib.metadata

_DISTRIBUTION_METADATA = importlib.metadata.metadata('dme')
...
class MainBase(object):
    ...
    def main_args(self):
        parser = argparse.ArgumentParser(
            prog="dme main", description="{} - {} - Main".format(APP_NAME, _DISTRIBUTION_METADATA['Version']))
    ...
    def parser_args(self):
        parser = dme.Parser.create_parser()
        return parser

    def parser_run(self, args):
        dme.Parser.api_main(args)
    ...
    def auto_load_args(self):
        parser = dme.auto_load.create_parser()
        return parser

    def auto_load_run(self, args):
        dme.auto_load.main(args)
    ...
    def kmer_build_args(self):
        parser = dme.build_kmer_sets.create_parser()
        return parser

    def kmer_build_run(self, args):
        dme.build_kmer_sets.main(args)
    ...
    def kmer_query_args(self):
        parser = argparse.ArgumentParser(
            description='Tests sequenes using HMDM*kmers')
        parser.add_argument('-i', '--input', dest="input", required=True,
                            help="Input file (bam file from DME*BWT, json file of DME results, \
            fasta file of sequences)")
        parser.add_argument('--bwt', action="store_true",
                            help="Specify if the input file for analysis is a bam file generated from DME*BWT")
        parser.add_argument('--dme', action="store_true",
                            help="Specify if the input file is a DME results json file")
    ...
    def kmer_query_run(self, args):
        obj = CARDkmers(args.input, args.bwt, args.dme, args.fasta, args.k,
    ...
    def card_annotation_args(self):
        parser = dme.card_annotation.create_parser()
        return parser

    def card_annotation_run(self, args):
        dme.card_annotation.main(args)
    ...
    def wildcard_annotation_args(self):
        parser = dme.wildcard_annotation.create_parser()
        return parser

    def wildcard_annotation_run(self, args):
        dme.wildcard_annotation.main(args)
    ...
    def baits_annotation_args(self):
        parser = dme.baits_annotation.create_parser()
        return parser

    def baits_annotation_run(self, args):
        dme.baits_annotation.main(args)
    ...
    def remove_duplicates_args(self):
        parser = dme.remove_duplicates.create_parser()
        return parser

    def remove_duplicates_run(self, args):
        dme.remove_duplicates.main(args)
    ...
    def bwt_args(self):
        parser = argparse.ArgumentParser(
            prog="dme bwt", description='Aligns metagenomic reads to HMDM and wildHMDM reference using bowtie or bwa and provide reports.')
        parser.add_argument('-1', '--read_one', required=True,
                            help="raw read one (qc and trimmied)")
        parser.add_argument('-2', '--read_two',
                            help="raw read two (qc and trimmied)")
        parser.add_argument('-a', '--aligner', default="kma",
                            choices=['bowtie2', 'bwa', 'kma'], help="aligner")
        ...
        parser.add_argument('--include_wildcard', dest="include_wildcard",
                            action="store_true", help="include wildcard")
        parser.add_argument('--include_baits', dest="include_baits",
                            action="store_true", help="include baits")
        parser.add_argument('--mapq', dest="mapq",
                            help="filter reads based on MAPQ score")
        parser.add_argument('--mapped', dest="mapped",
                            help="filter reads based on mapped reads")
    ...
    def tm_args(self):
        parser = argparse.ArgumentParser(prog="dme tm", description='TM')
    ...
    def heatmap_args(self):
        parser = argparse.ArgumentParser(
            prog="dme heatmap", description='Creates a heatmap when given multiple DME results.')
        parser.add_argument('-i', '--input', dest="input", required=True,
                            help="Directory containing the DME .json files (REQUIRED)")
        ...
        parser.add_argument('-o', '--output', dest="output", default="DME_heatmap", help="Name for the output EPS and PNG files. \
            The number of files run will automatically be appended to the end of the file name.\
            (default={})".format('DME_heatmap'))
    ...
    def clean_args(self):
        parser = dme.clean.create_parser()
        return parser
    ...
    def galaxy_args(self):
        parser = argparse.ArgumentParser(prog="dme galaxy", description="{} - {} - Galaxy project wrapper".
                                         format(APP_NAME, _DISTRIBUTION_METADATA['Version']), epilog=GALAXY_PROJECT_WRAPPER)
        parser.add_argument(
            '--galaxy_database', help='path to HMDM data and blast databases', required=True)
        parser.add_argument('--debug', dest="debug",
                            action="store_true", help="debug mode")
        return parser
    ...
    def database_args(self):
        parser = argparse.ArgumentParser(
            prog="dme database", description="{} - {} - Database".format(APP_NAME, _DISTRIBUTION_METADATA['Version']))
        parser.add_argument('-v', '--version', action='store_true',
                            required=True, help="prints data version number")
        parser.add_argument('--local', dest="local_database", action='store_true',
                            help="use local database (default: uses database in executable directory)")
        parser.add_argument('--all', action='store_true',
                            help="data version number used for `dme bwt` and `dme main` (default: dme main)")
        return parser
    ...
    def database_run(self, args):
        ...
        if args.local_database:
            ...
            if not os.path.exists(LOCAL_DATABASE):
                ...
                print(
                    "Please run `dme load --local -i <path to hmdm.json>` to create local database.")
                print("See `dme load --help` to upload the hmdm.json to dme application.\n".format(
        ...
        if os.path.isfile(indecies_directory) == True:
            with open(indecies_directory) as json_file:
                ...
                if args.all == True:
                    kmers_str = ",".join(json_data["hmdm_kmers"]["kmer_sizes"])
                    if kmers_str == "":
                        kmers_str = "N/A"
                    data_version = ("hmdm_canonical: {} | hmdm_variants: {} | kmer_sizes: {}".format(
                        json_data["hmdm_canonical"]["data_version"],
                        json_data["hmdm_variants"]["data_version"],
                        kmers_str
                    )
                    )
                else:
                    data_version = json_data["hmdm_canonical"]["data_version"]
        else:
