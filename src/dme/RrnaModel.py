...
class Rrna(BaseModel):
    ...
    def run(self):
        ...
        if os.stat(os.path.join(self.xml_file)).st_size > 0:
                ...
                for blast_record in blast_records:
                    ...
                    for alignment in blast_record.alignments:
                            ...
                            for hsp in alignment.hsps:
                                ...
                                for eachs in snp_dict_list:
                                    ...
                                    if hsp.query_start < pos and (hsp.query_start + real_query_length) > pos:
                                        ...
                                        qry = int(
                                            pos) - hsp.query_start + self.find_num_dash(hsp.query, (int(pos) - hsp.query_start))
                                        sbj = int(
                                            pos) - hsp.query_start + self.find_num_dash(hsp.query, (int(pos) - hsp.query_start))
                                        ...
                                        if hsp.sbjct[sbj] == chan:
                                            query_snps = {}
                                            logger.info(
                                                "hsp.query_start: {}".format(hsp.query_start))
                                            logger.info(
                                                "hsp.query_end: {}".format(hsp.query_end))
                                            logger.info(
                                                "hsp.sbjct_start: {}".format(hsp.sbjct_start))
                                            logger.info(
                                                "hsp.sbjct_end: {}".format(hsp.sbjct_end))
                                            d = 0
                                            if strand == "+":
                                                d = int(
                                                    pos) - hsp.query_start - self.find_num_dash(hsp.sbjct, (int(pos) - hsp.query_start))
                                                # d = -1*(hsp.query_start - pos)
                                                query_snps = {
                                                    "original": hsp.query[d], "change": hsp.sbjct[d], "position": (d + 1)}
                                            else:
                                                d = int(
                                                    pos) - hsp.query_start - self.find_num_dash(hsp.sbjct, (int(pos) - hsp.query_start))
                                                # d = -1*(hsp.query_start - pos)
                        ...
                        blastResults = self.results(
                            blastResults, blast_record.query + " | QUERY: " + alignment.hit_def, perfect, strict, loose)
