...
def clean():
    ...
    for datafile in data_files:
        if os.path.isfile(datafile) and os.path.basename(datafile) not in ["loaded_databases.json", "hmdm.json", ".gitignore", "__init__.py"]:
...
def clean_local():
    if os.path.exists(LOCAL_DATABASE):
        ...
        for f in files:
            if os.path.isfile(f) and os.path.basename(f) not in ["hmdm.json"]:
                logger.info("Remove: {}".format(f))
                os.remove(f)
        ...
        logger.warning(
            "Local database not found at {}, nothing to clean.".format(LOCAL_DATABASE))
...
def create_parser():
    parser = argparse.ArgumentParser(
        prog="rgi clean", description="{} - {} - Clean".format(APP_NAME, SOFTWARE_VERSION))
