...
from dme.HomologModel import Homolog
from dme.VariantModel import Variant
from dme.OverexpressionModel import Overexpression
from dme.RrnaModel import Rrna
...
class Filter(BaseModel):
    ...
    """This class takes in blast xml file and hmdm.json file and producess perfect strict paradigm for DME """

    def __init__(self, input_type, loose, input_sequence, xml_file, hmdm_json, input_file, output_file, num_threads, dme_obj=None):
        self.input_type = input_type
        self.xml_file = xml_file
        self.hmdm_json = hmdm_json
        self.input_filename = input_file
        self.input_sequence = input_sequence
        self.loose = loose
        self.blast_results = {}
        self.rna_results = {}
        self.dme_obj = dme_obj
        self.num_threads = num_threads
        self.working_directory = dme_obj.working_directory
        ...
        try:
            if model_type == "homolog":
                obj = Homolog(self.input_type, self.loose, self.input_sequence, self.xml_file,
                              self.working_directory, self.dme_obj.local_database, self.dme_obj.include_nudge)
            if model_type == "variant":
                obj = Variant(self.input_type, self.loose, self.input_sequence,
                              self.xml_file, self.working_directory, self.dme_obj.local_database)
            if model_type == "overexpression":
                obj = Overexpression(self.input_type, self.loose, self.input_sequence,
                                     self.xml_file, self.working_directory, self.dme_obj.local_database)
            results = obj.run()
    ...
    def process_rrna(self, model_type="rrna"):
        ...
        if self.input_type == "protein":
            ...
            rrna_obj = Rrna(self.input_sequence, self.output_file,
                            d, x, self.loose, self.dme_obj.local_database)
            res = rrna_obj.run()
    ...
    def create_db_query(self):
        ...
        db_obj = Database(self.dme_obj.local_database)
    ...
    def blast_reference_to_db_query(self, db, xml_file):
        ...
        rrna_db_fasta = os.path.join(self.dme_obj.db, "rnadb.fsa")
        blast_obj = Blast(rrna_db_fasta, program='blastn', output_file=xml_file,
                          local_database=self.dme_obj.local_database, num_threads=self.num_threads)
    ...
    def run(self):
        if (os.path.exists(self.xml_file)):
            ...
            logger.error("missing blast xml file({}). Please check if input_type: '{}' correspond with input file: '{}' or use '--low_quality' flag for short contigs to predicts partial genes.".format(
                self.xml_file, self.input_type, self.input_sequence))
