...
class Blast(object):
	...
	def __init__(self,input_file, output_file=None, program='blastp', num_threads=32, outfmt=5, local_database=False):
	...
	def run(self):
		...
		if self.program in ['blastn', 'tblastn', 'tblastx']:
			os.system('{program} -query {input} -db {path} \
						...
						.format(
							...
							path=os.path.join(self.db,"dna.db"),
